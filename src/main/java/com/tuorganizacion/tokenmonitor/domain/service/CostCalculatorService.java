...
public class CostCalculatorService {
    ...
    public CostCalculationResult calculateCost(String provider, int inputTokens, int outputTokens, String model) {
        ...
        if (!pricingRegistry.containsKey(providerKey)) throw new IllegalArgumentException("Proveedor no soportado");
        Map<String, ModelPricing> providerModels = pricingRegistry.get(providerKey);
        if (!providerModels.containsKey(modelKey)) throw new IllegalArgumentException("Modelo no soportado");

        ModelPricing pricing = providerModels.get(modelKey);
        BigDecimal inputCost = pricing.inputPricePer1k().multiply(new BigDecimal(inputTokens)).divide(new BigDecimal(1000), 6, RoundingMode.HALF_UP);
        BigDecimal outputCost = pricing.outputPricePer1k().multiply(new BigDecimal(outputTokens)).divide(new BigDecimal(1000), 6, RoundingMode.HALF_UP);
        return new CostCalculationResult(providerKey, modelKey, inputTokens, outputTokens, inputCost.add(outputCost));
    }
}
